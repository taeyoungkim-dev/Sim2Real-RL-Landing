...
# ==========================================
class ArucoUltimateLandNode(Node):
    def __init__(self):
        super().__init__('aruco_ultimate_land_node')
        ...
        self.current_x = 0.0
        self.current_y = 0.0
        self.current_z = 0.0 # NED 좌표계 (-10.0 이 10m 상공)
    ...
    def timer_callback(self):
        ...
        if self.timer_count == 20:
            ...
            self.get_logger().info("3단계: 목표 고도 10m로 상승 시작!")

        elif self.flight_state == "CLIMBING":
            # 실제 고도가 9.5m (NED -9.5) 이상 도달했을 때 하강 시작
            if self.current_z <= -9.5: 
                self.flight_state = "DESCENDING"
                self.get_logger().info("4단계: 10m 도달. 마커 추적 하강 시작 🚁⬇️🎯")
            ...
            if self.current_z > -0.5: 
                self.get_logger().info("5단계: 지면 접근 완료. 자동 착륙(Land) 🛑")
    ...
    def publish_trajectory_setpoint(self):
        ...
        if self.flight_state in ["INIT", "CLIMBING"]:
            # 상승: 이륙 위치(0,0) 유지하며 10m 고도로 위치 제어
            ...
            if self.marker_visible:
                ...
                msg.velocity = [self.target_vx, self.target_vy, 0.5] # Vz=0.5 (하강)
            else:
                # 🌟 마커 놓쳤을 때 (핵심!): X, Y는 위치(Position) 고정, Z만 속도(Velocity) 하강
                msg.position = [self.hold_x, self.hold_y, np.nan]
                msg.velocity = [np.nan, np.nan, 0.5] # Vz=0.5 (하강)
